...
| Settings 入口可见 | header 有 ⚙️ 按钮 | 浏览器截图验证 | ✅ 按钮可见且可点击 |
| 配置表单渲染 | 显示 llm/tts 等节点卡片 | 浏览器截图验证 | ✅ 显示 LLM(7字段) + TTS(1字段) |
| api_key 显示脱敏 | 输入框显示 `****` | 浏览器检查 | ✅ 显示 `••••` 密码遮罩 |
