...
def parse_story_direct(
    ...
    # ---- System Prompt ----
    chars_hint = f"\n【已知角色清单】（从故事末尾提取，包含所有出场角色）：{', '.join(known_chars)}" if known_chars else ""
...
2. **优先以『』识别对话**，被『』包裹的文字算对白，『』外归旁白
3. **『』外的内容全部归旁白**：场景描写、动作描写、表情描写、心理活动、叙述性过渡全部归旁白，role为"旁白"
4. **角色的 text 只包含『』内的纯对白**，不能混入"他笑了笑""她走上前""XX说"等叙述
5. **旁白的 text 包含所有非对白叙述**，保留原文
6. **角色名优先从已知角色清单匹配**，清单外的角色从原文中提取说话人名字
