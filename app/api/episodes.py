...
async def api_generate_dialogues(project_id: str, episode_id: str, body: DialogueGenRequest):
    """用 LLM 根据剧集摘要生成讲故事风格的白话叙事（含旁白+对话+场景描写）。"""
    proj = get_project(project_id)
    if not proj:
        raise HTTPException(404, "Project not found")
    ...
    num_scenes = max(3, min(8, target_total // 40))  # 每幕约 40 条，3~8 幕
...
- 角色的行为、对话风格、情感反应必须符合其性格特征和基础风格"""
    ...
    try:
        ...
        raise HTTPException(502, f"LLM 幕规划失败: {e}")
    ...
    sys.stderr.flush()
    if not scenes_plan:
        raise HTTPException(502, "LLM 未返回幕结构")
    ...
    for scene_i, scene in enumerate(scenes_plan):
        ...
        # 补生成循环：每幕分多轮，每轮最多 20 条，每轮最多重试 2 次
        ...
        if scene_collected:
            ...
            scene_return_summary = scene_summary
            completed_scenes_summary.append(scene_return_summary)
    ...
    created = []
    new_chars = []
    # 本次生成中新建的角色名→id 缓存，避免同一次调用内重复创建
